...
class CustomComparator : public rocksdb::Comparator {
public:
  CustomComparator() {}
  ~CustomComparator() {}
...
int perform_read(int id, rocksdb::DB *db, int count, uint8_t klen, uint32_t vlen, char *keys, int key_mode, int rd_nums, std::unordered_map<int, int>& rd_lat) {
  ...
  for (int i = start_key; i < start_key + rd_nums; i++) {
    ...
    rocksdb::Status ret = db->Get(rdopts, db_key, &val);;
...
void do_io(int id, rocksdb::DB *db, int count, uint8_t klen, uint32_t vlen, int op_type, char *keys, int rd_nums, int key_mode, int key_offset, std::unordered_map<int, int>& rd_lat) {
