...
int perform_read(int id, wisckey::DB *db, int count, uint8_t klen, uint32_t vlen, char *keys, int key_mode, int rd_nums, std::unordered_map<int, int>& rd_lat) {
  ...
  for (int i = start_key; i < start_key + rd_nums; i++) {
    ...
    wisckey::Status ret = db->Get(rdopts, db_key, &val);;
...
int perform_insertion(int id, wisckey::DB *db, int count, uint8_t klen, uint32_t vlen, char *keys, int key_mode, int key_offset) {
  ...
  for(int i = start_key; i < start_key + count; i++) {
    ...
    wisckey::Slice db_key(key, klen);
    wisckey::Slice db_val(value, vlen);

    wisckey::Status ret = db->Put(wropts, db_key, db_val);
    if( !ret.ok() ) {
...
void do_io(int id, wisckey::DB *db, int count, uint8_t klen, uint32_t vlen, int op_type, char *keys, int rd_nums, int key_mode, int key_offset, std::unordered_map<int, int>& rd_lat) {
...
int main(int argc, char *argv[]) {
  ...
  wisckey::DB *db = NULL;
  wisckey::DB::Open(options_, dev_path, &db);
