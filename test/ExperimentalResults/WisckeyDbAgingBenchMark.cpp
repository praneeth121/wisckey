...
int main(int argc, char *argv[]) {
  ...
  wisckey::Status status = wisckey::DB::Open(options_, "wisckeydb_benchmark", &db);
  if (status.ok())
    printf("wisckey open ok\n");
  else
    printf("wisckey open error\n");
...
#pragma omp atomic
        ...
        } else if (op == 1) { // perform delete
#pragma omp atomic
          delete_keys++;
          wisckey::Slice delete_value("",0);
          wisckey::Status ret = db->Put(wropts, db_key, delete_value);
          if (!ret.ok()) {
            printf("something wrong in delete\n");
          }
        } else if (op == 2) { // perform insert newkey
#pragma omp atomic
          insert_keys++;
#pragma omp atomic
          new_key++;
  ...
  wisckey::Iterator *it = db->NewIterator(options);
  delete db;
  return 0;
}