...
void *get_database(char *db_type) {
  if (strcmp(db_type, "new") == 0) {
    ...
    return (void *)db;
  } else if (strcmp(db_type, "wisc") == 0) {
    wisckey::Options options;
    options.statistics = wisckey::Options::CreateDBStatistics();
    wisckey::DB *db = NULL;
    wisckey::DB::Open(options, "./wisckey_benchmark/", &db);
    return (void *)db;
  } else if (strcmp(db_type, "rocks") == 0) {
    rocksdb::Options valuedbOptions;
    valuedbOptions.IncreaseParallelism();
    valuedbOptions.create_if_missing = true;
    valuedbOptions.max_open_files = -1;
    ...
    valuedbOptions.max_bytes_for_level_base = 64 * 1048576;
    rocksdb::DB *db = NULL;
    rocksdb::DB::Open(valuedbOptions, "./rocksdb_benchmark/", &db);
    return (void *)db;
  }
  return NULL;
}
...
int main(int argc, char *argv[]) {
  ...
  if (db_type == NULL) {
    fprintf(stderr, "Please specify DB TYPE\n");
    usage(argv[0]);
    return SUCCESS;
  }

  void *db_ = get_database() if (strcmp(db_type, "wisc") == 0) {
    wisckey::DB *db = (wisckey::DB *)db_;
  }
  else if (strcmp(db_type, "rocks") == 0) {
    rocks::DB *db = (rocks::DB *)db_;
  }
  else {
    newdb::DB *db = (newdb::DB *)db_;
  }
  ...
  // std::thread stat_thread(stats_thread, total_ops, STATS_POOL_INT, 1, 10,
  // stats_mode);
...
#pragma omp parallel for num_threads(t)
  for (int i = 0; i < t; i++) {
    ...
    for (int i = start_key; i < start_key + count; i++) {
      ...
      ops_keys.fetch_add(1, std::memory_order_relaxed);
    }
    if (key)
      free(key);
    if (value)
  ...
  printf("Loaded the database with keys\n");
