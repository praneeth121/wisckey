...
void DBIterator::prefetch_value(std::vector<int> fd_list,
  ...
  for (int i = 0; i < prefetch_num; i++) {
    ...
    async_pread(fd_list[i], aligned_val_buf_list[i], rSize, aligned_raddr,
                on_prefetch_complete, (void *)ctx);
  }
...
DBIterator::DBIterator(DBImpl *db, const ReadOptions &options)
  ...
  if (options_.upper_key != NULL) {
    ...
    // printf("upper key: %s\n", std::string(s_upper_key_.data(),
    // s_upper_key_.size()).c_str());
  }
...
Slice DBIterator::value() {
  ...
  if (prefetch_ena_) {
    ...
    rocksdb::Status s =
        db_->rdb_->Get(rocksdb::ReadOptions(), rocks_key, &rocks_val);
    ...
    size_t rret =
        pread(db_->logFD_[hash], aligned_val_buf, rSize, aligned_raddr);
