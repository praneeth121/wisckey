...
private:
  ...
  Cache::Handle *read_buffer(int id, std::string &key, std::string *value) {
    if (write_buffer_[id] == NULL)
      return NULL;
    Cache::Handle *h = write_buffer_[id]->Lookup(key);
    if (h != NULL) {
      CacheEntry *rd_val =
          reinterpret_cast<CacheEntry *>(write_buffer_[id]->Value(h));
  ...
  Cache::Handle *insert_buffer(int id, std::string &key, const Slice &value) {
    ...
    Cache::Handle *h =
        write_buffer_[id]->Insert(key, reinterpret_cast<void *>(ins_val),
                                  charge, DeleteEntry<CacheEntry>);
  ...
  void erase_buffer_entry(int id, std::string &key) {
    ...
    if (evicted)
      RecordTick(options_.statistics.get(), WBUFFER_ERASE);
  };
  ...
  Cache::Handle *insert_cache(std::string &key, const Slice &value) {
    ...
    CacheEntry *ins_val = new CacheEntry((char *)value.data(), value.size());
    size_t charge = sizeof(CacheEntry) + value.size();
    Cache::Handle *h = cache_->Insert(key, reinterpret_cast<void *>(ins_val),
                                      charge, DeleteEntry<CacheEntry>);
