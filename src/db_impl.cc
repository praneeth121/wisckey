...
DBImpl::DBImpl(const Options &options, const std::string &dbname)
  ...
  if (options.filterType == Bloom) {
    table_options.filter_policy.reset(
        rocksdb::NewBloomFilterPolicy(options.filterBitsPerKey, true));
  ...
  //   options.filterBitsPerKey);
  //   table_options.filter_policy.reset(rocksdb::NewSuRFPolicy(2, 4, true,
  //   options.filterBitsPerKey, true));
  // }
  ...
  if (options.indexCacheSize > 0)
    table_options.block_cache =
        rocksdb::NewLRUCache((size_t)options.indexCacheSize * 1024 * 1024LL);
  else {
  ...
  rocksOptions.table_factory.reset(
      rocksdb::NewBlockBasedTableFactory(table_options));

  // allocate write buffer 2 times larger than log buffer
  for (int i = 0; i < LOG_PARTITION; i++) {
    write_buffer_[i] =
        NewFIFOCache((size_t)options.logBufSize / LOG_PARTITION * 2 + 16384, 2);
  }
  ...
  for (int i = 0; i < LOG_PARTITION; i++) {
    ...
    wal_flush_cnt_[i] = 0;
    // wal
    std::string dbWiskeyWAL =
        dbname + "/wiskeyWAL" + std::to_string(i) + ".log";
    walFD_[i] = fopen(dbWiskeyWAL.c_str(), "wb");

    if (options.logBufSize > 0) {
      aligned_log_buf_[i] =
          (char *)aligned_alloc(PAGE_SIZE, options.logBufSize / LOG_PARTITION);
    }

    std::string dbWiskeyLog = dbname + "/wiskey" + std::to_string(i) + ".log";
    if (access(dbWiskeyLog.c_str(), F_OK) == 0) {
      logFD_[i] =
          open(dbWiskeyLog.c_str(), O_RDWR | O_APPEND | O_DIRECT | O_SYNC);
      size_t fileSize = lseek(logFD_[i], 0, SEEK_END);
      lba_[i] = fileSize;
      std::cout << "Open existing Wisckey log path:  " << dbWiskeyLog << " ("
                << logFD_[i] << ")" << std::endl;
    } else {
      logFD_[i] = open(dbWiskeyLog.c_str(),
                       O_RDWR | O_CREAT | O_APPEND | O_DIRECT | O_SYNC, 0777);
      lba_[i] = 0;
      std::cout << "Create Wisckey log path:  " << dbWiskeyLog << " ("
                << logFD_[i] << ")" << std::endl;
    }
  }
}
...
DBImpl::~DBImpl() {
  ...
  // if (log_buf_offset_) { // here log_buf_offset will not overflow (value size
  // 4KB)
  ...
  for (int i = 0; i < LOG_PARTITION; i++) {
    ...
    if (aligned_log_buf_[i])
      free(aligned_log_buf_[i]);
    fsync(logFD_[i]);
    fdatasync(logFD_[i]);
    close(logFD_[i]);
  }
...
Status DBImpl::Put(const WriteOptions &options, const Slice &key,
  ...
  //     std::string dbWiskeyWAL = dbname_+"/wiskeyWAL" + std::to_string(hash) +
  //     ".log"; fclose(walFD_[hash]); walFD_[hash] = fopen(dbWiskeyWAL.c_str(),
  //     "wb"); wal_flush_cnt_[hash] = 0;
  //   }
  // }
  ...
  // write value to log, make sure lseek and write atomic
  {
    ...
    if (log_buf_offset_[hash] >=
        options_.logBufSize / LOG_PARTITION -
            PAGE_SIZE) { // here log_buf_offset will not overflow (value size
                         // 4KB)
      // pwrite needs to write PAGE_SIZE aligned buffer
      log_buf_offset_[hash] = roundUp(log_buf_offset_[hash], PAGE_SIZE);
      wret = pwrite(logFD_[hash], aligned_log_buf_[hash], log_buf_offset_[hash],
                    lba_[hash]);
      // fprintf(stderr, "pwrite fid: %d, size %lu, lba %lu, ret %d\n",
      // logFD_[hash], log_buf_offset_[hash], lba_[hash], wret); printf("pwrite
      // size %lu, lba %lu, ret %d\n", log_buf_offset_[hash], lba_[hash], wret);
...
Status DBImpl::Get(const ReadOptions &options, const Slice &key,
  ...
  // fprintf(stderr, "[pread] fid: %d, offset: %lu, size: %lu, read: %d, errno:
  // %d\n", logFD_[hash], aligned_raddr, rSize, rret, errno); printf("[pread]
  // offset: %lu, size: %lu, read: %d, errno: %d\n", aligned_raddr, rSize, rret,
  // errno);

  int pr_retry_cnt = 0;
  while (rret <= 0) { // read retry (log buffer not flushed, something wrong
                      // with write buffer), rare
    usleep(100);
    rret = pread(logFD_[hash], aligned_val_buf, rSize, aligned_raddr);
    fprintf(
        stderr,
        "[pread retry] fid: %d, offset: %lu, size: %lu, read: %d, errno: %d\n",
...
void DBImpl::flushVLog() {
  ...
  for (int i = 0; i < LOG_PARTITION; i++) {
    ...
    if (log_buf_offset_[i]) {
      ...
      wret =
          pwrite(logFD_[i], aligned_log_buf_[i], log_buf_offset_[i], lba_[i]);
  ...
  for (int i = idx; i < idx + size; i++) {
    ...
    // write value to log, make sure lseek and write atomic
    {
      ...
      if (log_buf_offset_[hash] >=
        ...
        // pwrite needs to write PAGE_SIZE aligned buffer
        log_buf_offset_[hash] = roundUp(log_buf_offset_[hash], PAGE_SIZE);
        wret = pwrite(newLogFD[hash], aligned_log_buf_[hash],
                      log_buf_offset_[hash], lba_[hash]);
        lba_[hash] += log_buf_offset_[hash];
        log_buf_offset_[hash] = 0;
      }
    }
...
// Just to be safe, read all live value metadata (offset, vsize) into memory
// through iterator, then update the lsm-tree
void DBImpl::vLogGarbageCollect() {
  ...
  for (int i = 0; i < LOG_PARTITION; i++) {
    ...
    // open new vlog
    newLogFD_[i] = open(old_vlog.c_str(),
                        O_RDWR | O_CREAT | O_APPEND | O_DIRECT | O_SYNC, 0777);
    lba_[i] = 0;
    std::cout << "Create new Wisckey log path:  " << old_vlog << " ("
              << newLogFD_[i] << ")" << std::endl;
  }
  ...
  for (int p = 0; p < LOG_PARTITION; p++) {
    ...
    for (int i = 0; i < gc_worker_num; i++) {
      ...
      gc_worker_threads[i] = new std::thread(
          &wisckey::DBImpl::vLogGCWorker, this, p, &ukey_list[p],
          &vmeta_list[p], job_start_idx, jSize, oldLogFD_, newLogFD_);
      job_start_idx += jSize;
      remain_keys -= jSize;
    }
  ...
  std::cout << std::fixed << "GC Wall clock time passed: "
  ...
  // allocate write buffer 2 times larger than log buffer
  for (int i = 0; i < LOG_PARTITION; i++) {
    write_buffer_[i] = NewFIFOCache(
        (size_t)options_.logBufSize / LOG_PARTITION * 2 + 16384, 2);
  }
